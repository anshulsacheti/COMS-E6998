...
def findSeedSet(checkinDF, G, budget, conversionRate, longitude, latitude, runType, checkins, neighborMinimum, CheckinsPerLocation):
    ...
    if runType=="randomWalk":
        ...
        for node in graphNodes:
            neighborsPerNode[node] = list(G.neighbors(node))

        seedNode = np.random.choice(graphNodes)
        seedSet = [seedNode]
        ...
        for i in range(budget-1):
            ...
            while nodeNotAdded:
                for j in range(2):
                    neighbors = neighborsPerNode[seedNode]
                    seedNode = np.random.choice(neighbors)

                if seedNode not in seedSet:
                    nodeNotAdded = False
    ...
    if runType=="Randomized":
        ...
        while len(seedSet)<budget:
            node = np.random.choice(graphNodes)
            if node not in seedSet:
                seedSet.append(node)
...
if __name__ == "__main__":
    ...
    for i in range(iters):
        ...
        profit, Anodes = simulate_graph.simulate(G, seedsA, seedsB, conversionRate)
        totalProfit += profit
